...
def sales_report():
    ...
    try:
        sale = pickle.load(f)
        while sale:
            print(sale_report_line.format(sale['sku'], sale['quantity'],
                                          sale['total']))
            sale = pickle.load(f)
    except EOFError:
...
def transaction():
    ...
    if sku in inventory:
        ...
        try:
            ...
            print(e, 'Could not complete sale!')
    else:
...
def inventory_management():
    ...
    for k in keys:
        print(inventory_report_line.format(k, inventory[k]['name'],
    ...
    if sku in inventory:
        product = inventory[sku]
        try:
            print('SKU', sku, ':', product['name'])
            product['quantity'] = int(input('Update quantity on hand >>> '))
            product['price'] = float(input('Update price >>> '))
        except ValueError as e:
            print(e, 'Failed to update', sku)
    else:        
        product = {}
        product['name'] = input('Enter name for SKU {} > '.format(sku))
        try:
            product['quantity'] = int(input('Enter quantity on hand >>> '))
